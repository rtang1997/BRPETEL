...
X=mvrnorm(n,c(1,2,3),matrix(c(1,0.5,0.3,0.4,2,0.8,0.8,0.9,3),nrow=3))
for(i in 1:n)
{X[i,]=X[i,]/as.numeric(sqrt(t(X[i,])%*%X[i,]))}

 
    
##################### choose covariance matrix I used in RRWM algorithm ###############
...
if(true_u<=u1)
 ...
 if(iter%%10==0)
 {print(c(covpp95,covpp95_1,covpp95_2,covpp95_3))}
}
